...
angular
    ...
    .controller("UsersLoginController",['$scope', '$http', '$httpParamSerializerJQLike', function ($scope, $http, $httpParamSerializerJQLike) {
        $scope.post = {
            remember_me: 1
        };
        ...
        var isOAuthResponse = function (hasSsoError) {
            ...
            if (query.hasOwnProperty('code') && query.hasOwnProperty('state')) {
                ...
                window.location.href = $scope.getLocalStorageItemWithDefaultAndRemoveItem('lastPage', '/');
            }
