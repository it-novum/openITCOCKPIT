...
class DebugShell extends AppShell {
	...
	public function main(){
		...
		$menuSelection = strtoupper($this->in(__d('oitc_console', 'What would you like to debug?'), array('D', 'Q')));
	...
	private function monitoringMenu(){
		...
		$menuSelection = strtoupper($this->in(__d('oitc_console', 'What would you like to do?'), array('T', 'TF', 'H', 'HT', 'S', 'ST', 'TP', 'CM', 'C', 'CG', 'HE', 'UUID', 'B')));
		$this->DebugConfigNagios->setup($this->conf);
		switch ($menuSelection){
			case 'T':
				$this->DebugConfigNagios->parseMonitoringLogfile();
