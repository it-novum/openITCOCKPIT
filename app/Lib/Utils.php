 ...
 */
	public static function tokenize($string) {
		return str_replace(array('/', '+', '='), array('_', '-', '~'), base64_encode(Security::cipher($string, Configure::read('Security.salt'))));
	}
 ...
 */
	public static function detokenize($string) {
		return Security::cipher(base64_decode(str_replace(array('_', '-', '~'), array('/', '+', '='), $string)), Configure::read('Security.salt'));
	}
 ...
 */	
	public static function htmlToXls($worksheetName, $tableMarkup) {
		...
		return String::insert($data, array(
			'name' => $worksheetName,
			'table' => $tableMarkup
		));
	}
 ...
 */
	public static function calculateFinalScore($match) {
		...
		if (in_array($match['status'], array(Status::MATCH_PRE_MATCH, Status::MATCH_REVOKED, Status::MATCH_WITHDRAWN, Status::MATCH_POSTPONED, Status::MATCH_CANCELED, Status::MATCH_DISCARDED))) {
			return array(
				'teamHomeTotal' => 0,
				'teamAwayTotal' => 0,
				'winner' => false
		...
		if ($teamHomeTotal > $teamAwayTotal) {
			...
			$scores['winner'] = array($match['team_home_id'], $match['team_away_id']);
		}
