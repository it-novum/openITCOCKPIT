...
class ClearCachePanel extends DebugPanel {
     ...
     * @var array
     */
    public $folders = ['_all_'];

    /**
     * Allowed cache engines
     * @var array
     */
    public $engines = ['_all_'];
    ...
    public function __construct($settings) {
        ...
        $engines = array_diff(Cache::configured(), ['debug_kit']);
        $this->engines = array_merge($this->engines, $engines);

        $groups = array_keys(ClearCache::getGroups());
        $this->groups = empty($groups) ? [] : array_merge($this->groups, $groups);

        foreach (['folders', 'engines', 'groups'] as $property) {
