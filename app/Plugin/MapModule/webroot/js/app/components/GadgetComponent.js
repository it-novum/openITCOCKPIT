...
App.Components.GadgetComponent = Frontend.Component.extend({
	...
	drawTacho:function(svgContainerId, opt){
		...
		svg.path(needle,path.move(centerX,centerY).line(-5,0,true).line(2,-80, true).line(3,-10,true).line(3,10, true).line(2,80, true).close(),{
			stroke:'#000', strokeWidth:1, fill:'#3D3C3D', transform:'rotate('+rotatingValue+', '+centerX+', '+centerY+')'
		});
		//center circle
		...
		for (var i = 0; i < 101; i++) {
			...
			if(i%10==0){
				svg.line(scale, centerX, centerY-(radius-3), centerX, centerY-(radius-15),{
					stroke:'#000', strokeWidth: 1, transform:'rotate('+scaleRotatingValue+', '+centerX+', '+centerY+')'
				});
				if(perfdata[0]){
					if(showPercentScale){
		...
		if(containSVG){
			...
			if(containerData){
				...
				$(containerDiv).attr(data).css({'width':radius*2+'px','height':radius*2+'px'}).addClass('elementHover');
			}else{
	...
	drawCylinder:function(svgContainerId, opt){
		...
		if(containSVG){
			...
			if(containerData){
				...
				$(containerDiv).attr(data).css({'width':width+'px','height':height+'px'}).addClass('elementHover');
			}else{
				//there is no data given so create the div without hover information
	...
	drawText:function(svgContainerId, opt){
		...
		if(containSVG){
			...
			$('#'+svgContainerId).css({'height':parseInt(textSize.height)+'px', 'width':parseInt(textSize.width)+'px'});
			$('#'+svgContainerId).children('svg').attr({'width':parseInt(textSize.width), 'height':parseInt(textSize.height)});
			...
			if(containerData){
				...
				$(containerDiv).attr(data).css({'width':textSize.width+'px','height':textSize.height+'px'}).addClass('elementHover');
			}else{
				//there is no data given so create the div without hover information
				$(containerDiv).css({'width':textSize.width+'px','height':textSize.height+'px'}).addClass('elementHover');
			}
			$(foreignObject).attr({'x':textX,'y':textY-10,'width':textSize.width, 'height':textSize.height}).append(containerDiv);
	...
	drawTrafficLight:function(svgContainerId, opt){
		...
		//SVG Container
		$('#'+svgContainerId).css({'top':y+'px', 'left':x+'px','position':'absolute', 'height':sizeY, 'width': sizeX+40}).svg();
		var svg = $('#'+svgContainerId).svg('get');
...
*/
	...
	drawRRDGraph:function(svgContainerId, opt){
		...
		if(!demo){
			...
			var markerVert = svg.marker(defs, 'myMarker', 0, 5, 10, 10, 0,{fill: 'black', markerUnits: 'strokeWidth', viewBox: '0 0 20 20', fillOpacity: 0.3});
			svg.path(markerVert, arrow_path.move(0, 0).line(20, 5).line(0, 10).close());
			svg.line(element_group, startX, 170, startX, 10, {strokeWidth: 1,   stroke: 'black', markerEnd: 'url(#myMarker)', fillOpacity: 0.3});
			svg.path(markerVert, arrow_path.move(0, 0).line(20, 5).line(0, 10).close());
			svg.line(element_group, startX, 170, 280, 170, {strokeWidth: 1,   stroke: 'black', markerEnd: 'url(#myMarker)', fillOpacity: 0.3});
