...
App.Controllers.MapeditorsEditController = Frontend.AppController.extend({
	...
	_initialize: function() {
		...
		for (var i = 0; i < gadgets.length; i++) {
			...
			if(gadgets[i] in gadgetScale){
				...
				$('#'+gadgets[i]+'Thumbnail')
				.css({'-webkit-transform':'rotate('+gadgetScale[gadgets[i]].rotate+') scale('+gadgetScale[gadgets[i]].value+') translate('+gadgetScale[gadgets[i]].translate[0]+'px,'+gadgetScale[gadgets[i]].translate[1]+'px)',
		...
		$('#saveElementPropertiesBtn').click(function(){
			...
			if($('#'+self.current['elementUuid']).length > 0){
				...
				if(typeof(self.current['type']) !== 'undefined'){
					...
					el.addEventListener('dblclick', function(){self.editElements(this);});
		...
		$('#saveGadgetPropertiesBtn').click(function(){
			...
			if($('#'+self.currentGadget['elementUuid']).length > 0){
				...
				self.Gadget['draw'+self.currentGadget['gadget']]('svgContainer_'+self.currentGadget['elementUuid'], {id:self.currentGadget['elementUuid'],x:self.currentGadget['x'],y:self.currentGadget['y'], demo:true});
		...
		if(this.getVar('map_gadgets')){
			...
			for (var i = 0; i < mapGadgets.length; i++) {
				...
				self.Gadget['draw'+mapGadgets[i]['gadget']]('svgContainer_'+tempUuid, {id:tempUuid,x:mapGadgets[i]['x'],y:mapGadgets[i]['y'], containerData:containerData, demo:true});
	...
	changeBackground: function(opt){
		...
		if(!remove){
			...
			$('#MapBackground').val('');
			$mapContainer.css({'background-image':'none', 'background-repeat':'repeat', 'width':'100%', 'height':'100%'});
