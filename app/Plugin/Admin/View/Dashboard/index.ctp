			...
			<ul id="widget-tab-1" class="nav pull-left nav-tabs">
				...
				?>
					<?php if($tabId == $activeTabId):?>
						<li data-id="<?php echo $activeTabId; ?>" class="active dropdown-toggle dashboardTab" data-name="<?php echo h($tabName); ?>" position="<?php echo $position;?>">
					...
					<?php else: ?>
						<li data-id="<?php echo $activeTabId; ?>" data-name="<?php echo h($tabName); ?>" class="dashboardTab" position="<?php echo $position;?>">
				...
				<div class="btn-group">
					...
					<ul class="newTabsList dropdown-menu pull-right">
						...
						<li class="addSharedTab">
							<?php
							echo $this->Form->create('chooseSharedTab', array(
					...
					<ul class="dropdown-menu pull-right">
						<li>
							<div class="form-group form-group-slider <?php echo $this->CustomValidationErrors->errorClass('notification_interval'); ?>">
								<label class="col rotationSliderLabel" for="tabRotationInterval"><?php echo h('Choose tab rotation interval'); ?></label>
								<div class="col rotationSlider">
									<input type="text" id="tabRotationInterval" maxlength="255" value="" class="form-control slider slider-success" name="data[rotationInterval]" data-slider-min="0" data-slider-max="1200" data-slider-value="<?php echo $tabRotationInterval['User']['dashboard_tab_rotation'];?>" data-slider-selection="before" data-slider-step="<?php echo Configure::read('NagiosModule.SLIDER_STEPSIZE'); ?>" human="#HostNotificationinterval_human">
								</div>
								<div class="col rotationSliderInput">
									<input type="number" id="_tabRotationInterval" human="#HostNotificationinterval_human" value="<?php echo $tabRotationInterval['User']['dashboard_tab_rotation'];?>" slider-for="HostNotificationinterval" class="form-control slider-input" name="data[Host][notification_interval]">
									<span class="note" id="HostNotificationinterval_human"><?php echo $this->Utils->secondsInWords($this->CustomValidationErrors->refill('notification_interval', $tabRotationInterval['User']['dashboard_tab_rotation'])); ?></span>
		...
		<div class="newWidgetContainer" role="menu">
			<div class="btn-group">
				...
				<ul class="dropdown-menu pull-right">
					...
					?>
					<hr>
					<li widget-type="standard" class="addWidget"><a href="javascript:void(0);"><i class="fa fa-recycle">&nbsp;</i>Restore default</a></li>
				</ul>
			</div>
		</div>
	...
	<div class="gridstackContainer">
		<div class="grid-stack" data-gs-width="12" data-gs-animate="yes">
			<?php foreach($widgets as $widget):?>
				<div widget-id="<?php echo $widget['Widget']['id'];?>" class="grid-stack-item" data-gs-x="<?php echo $widget['Widget']['row'];?>" data-gs-y="<?php echo $widget['Widget']['col'];?>" data-gs-width="<?php echo $widget['Widget']['width']?>" data-gs-height="<?php echo $widget['Widget']['height']?>">
