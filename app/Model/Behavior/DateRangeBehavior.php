			...
			foreach($downtimes as $downtime) {
				...
				foreach(array_keys($timeperiodTimeslices) as $i) {
					...
					if($downtime['start_time'] > $time_slices[$i]['start'] && $downtime['end_time'] < $time_slices[$i]['end']) {
						$time_slices[] = array('start' => $downtime['end_time'], 'end' => $time_slices[$i]['end'], 'is_downtime' => false);
						$time_slices[$i]['end'] = $downtime['start_time'];
						if($show_outages_in_dowtime){
							$time_slices[] = array('start' => $downtime['start_time'], 'end' => $downtime['end_time'],'is_downtime' => true);
						}
						continue;
					}
					if($downtime['start_time'] <= $time_slices[$i]['start'] && $downtime['end_time'] >= $time_slices[$i]['end']) {
						...
						if($show_outages_in_dowtime){
							$time_slices[] = array('start' => $time_slices[$i]['start'], 'end' => $time_slices[$i]['end'],'is_downtime' => true);
						}
						unset($time_slices[$i]);
						continue;
					}
					if($downtime['start_time'] <= $time_slices[$i]['start'] && $downtime['end_time'] > $time_slices[$i]['start'] && $downtime['end_time'] < $time_slices[$i]['end']) {
						...
						if($show_outages_in_dowtime){
							$time_slices[] = array('start' => $tmp_start, 'end' => $downtime['end_time'],'is_downtime' => true);
						}
						continue;
					}
					if($downtime['end_time'] >= $time_slices[$i]['end'] && $downtime['start_time'] > $time_slices[$i]['start'] && $downtime['start_time'] < $time_slices[$i]['end']) {
						...
						$time_slices[$i]['end'] = $downtime['start_time'];
						if($show_outages_in_dowtime){
							$time_slices[] = array('start' => $downtime['start_time'], 'end' => $tmp_end,'is_downtime' => true);
						}
						continue;
					}
				}
