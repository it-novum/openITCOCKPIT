...
class qqFileUploader {
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        if ($name === null || $name === ''){
            return array('error' => __('upload.file_name_empty'));
        }

        // Validate file size

        if ($size == 0){
            return array('error' => __('upload.file_empty'));
        }

        if ($size > $this->sizeLimit){
            return array('error' => __('upload.file_too_big'));
        }
        ...
        if($this->allowedExtensions && !in_array(strtolower($ext), array_map("strtolower", $this->allowedExtensions))){
            ...
            return array('error' => sprintf(__('upload.invalid_file_type', true), $these));
        }
