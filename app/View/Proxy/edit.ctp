	...
	<?php echo $this->Form->create('Proxy', array(
		...
		<fieldset>
			...
			if(isset($proxy[0]['Proxy']['id'])):
				echo $this->Form->input('id', array('value' => $proxy[0]['Proxy']['id']));
			endif;
			?>
			<div class="row">
				<section class="col col-10">
					...
					<label class="input"> <i class="icon-prepend fa fa-exchange"></i>
						<?php
						$proxyconf = array('placeholder' => 'proxy.example.org');
						if(isset($proxy[0]['Proxy']['ipaddress'])):
							$proxyconf = array('value' => $proxy[0]['Proxy']['ipaddress']);
						endif;
				...
				<section class="col col-2">
					...
					<label class="input"> <i class="icon-prepend fa fa-terminal"></i>
						<?php
						$proxyconf = array('placeholder' => 3128);
						if(isset($proxy[0]['Proxy']['port'])):
							$proxyconf = array('value' => $proxy[0]['Proxy']['port']);
						endif;
				...
				<section class="col col-12 pull-right">
					...
					<label class="toggle">
						<input type="checkbox" <?php echo $checked; ?> name="data[Proxy][enabled]" value="true" />
							<i data-swchoff-text="<?php echo __('Off'); ?>" data-swchon-text="<?php echo __('On'); ?>"></i><?php echo __('Enable Proxy'); ?>
					</label>
				</section>
			</div>
		</fieldset>
