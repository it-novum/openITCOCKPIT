...
class AppFormHelper extends BoostCakeFormHelper{
	 ...
	 * @return string
	 */
	public function imageChooserInput($field, array $options = array()){
		$options = Hash::merge(array(
			'label' => Inflector::humanize($field)
		), $options);

		$hiddenInput = $this->input($field, array(
			'type' => 'hidden',
	 ...
	 * @return string
	 */
	public function fileChooserInput($field, array $options = array()){
		$options = Hash::merge(array(
			'label' => Inflector::humanize($field)
		), $options);
	...
	public function formActions($saveText = null, array $options = array()){
		...
		$out .= $this->submit($saveText, array('div' => false, 'class' => $options['saveClass']));
		if($options['cancelButton']){
			if(!is_array($options['cancelButton'])){
				$options['cancelButton'] = array();
				$options['cancelButton']['title'] = __('formactions.cancel');
				$options['cancelButton']['url'] = array('action' => 'index');
			}
			$out .= '&nbsp;' . $this->Html->link($options['cancelButton']['title'], $options['cancelButton']['url'], array('class' => 'btn btn-default'));
		}
		$out .= '</div>';
	...
	public function fancyCheckbox($fieldName, $options = array()){
		$this->_storedInputDefaults = $this->_inputDefaults;
		$this->Html->disableFieldset = false;
		$this->setEntity($fieldName);
		$__options = Hash::merge(array(
			'on' => __('On'),
			'off' => __('Off'),
		), $options);
	 ...
	 * @return string
	 */
	public function fancyCheckboxWithValue($fieldName, $options = array()){
		$this->_storedInputDefaults = $this->_inputDefaults;
		$this->Html->disableFieldset = false;
		$this->setEntity($fieldName);
		$__options = Hash::merge(array(
