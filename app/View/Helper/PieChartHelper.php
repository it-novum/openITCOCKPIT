...
class PieChartHelper extends AppHelper {
	...
	public function create($chart_data){
		...
		foreach($chart_data as $state => $state_count){
			...
			if($state_data_per['size'] === 0){
				$chart_data_array[$state] = array('color' => $colors[$state], 'startAngle' => $startAngle, 'endAngle' => $endAngle, 'show_percent_value' => $state_data_per['size']);
				continue;
			}
			$startAngle = $endAngle;
	...
	function circle3d($chart_data_array) {
		for ($i = $this->offset_3d ; $i > $this->y; $i--) {
			foreach($chart_data_array as $key => $data_arr){
				if($data_arr['show_percent_value']>0){
					imageSmoothArc($this->img, $this->x, $i, $this->width, $this->height,  array($data_arr['color'][0]*0.75,$data_arr['color'][1]*0.75,$data_arr['color'][2]*0.75,70), $data_arr['startAngle'], $data_arr['endAngle']);
				}
			}
		}
	}
