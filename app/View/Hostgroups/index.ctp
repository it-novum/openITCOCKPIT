				...
				<header>
					<div class="widget-toolbar" role="menu">
						...
						<ul class="dropdown-menu arrow-box-up-right pull-right">
							<li style="width: 100%;"><a href="javascript:void(0)" class="select_datatable text-left" class="select_datatable text-left" my-column="1"><input type="checkbox" class="pull-left" /> &nbsp; <?php echo __('Hostgroup name');?></a></li>
							...
							<li style="width: 100%;"><a href="javascript:void(0)" class="select_datatable text-left" class="select_datatable text-left" my-column="3"><input type="checkbox" class="pull-left" /> &nbsp; <?php echo __('Hosts'); ?></a></li>
						</ul>
							...
							<table id="hostgroup_list" class="table table-striped table-bordered smart-form" style="">
								<thead>
									<tr>
										...
										<th class="select_datatable no-sort"><?php echo $this->Utils->getDirection($order, 'Container.name'); echo $this->Paginator->sort('Container.name', 'Hostgroup name'); ?></th>
										<th class="no-sort"><?php echo $this->Utils->getDirection($order, 'Hostgroup.description'); echo $this->Paginator->sort('Hostgroup.description', 'Description'); ?></th>
										...
										<tr>
											<td class="text-center" style="width: 15px;">
												<?php if($this->Acl->hasPermission('edit') && $allowEdit): ?>
													<input class="massChange" type="checkbox" name="hostgroup[<?php echo $hostgroup['Hostgroup']['id']; ?>]" hostgroupname="<?php echo h($hostgroup['Container']['name']); ?>" value="<?php echo $hostgroup['Hostgroup']['id']; ?>"/>
												<?php endif; ?>
											</td>
											...
											<td>
												...
												foreach($hostgroup['Host'] as $host):
													...
													<li><a href="<?php echo Router::url(['controller' => 'hosts', 'action' => 'edit', $host['id']]); ?>"><?php echo h($host['name']);?></a></li>
												<?php else: ?>
											...
											<td>
												<center>
													<?php if($this->Acl->hasPermission('edit') && $allowEdit): ?>
														<a href="/<?php echo $this->params['controller']; ?>/edit/<?php echo $hostgroup['Hostgroup']['id']; ?>" data-original-title="<?php echo __('edit'); ?>"><i id="list_edit" class="fa fa-gear fa-lg txt-color-teal"></i></a>
													<?php endif;?>
												</center>
											</td>
										</tr>
