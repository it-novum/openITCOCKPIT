					...
					<?php foreach($hostgroups as $k => $hostgroup): ?>
						...
						<?php if(!empty($hostgroup['Host'])): ?>
							...
							<tr>
								...
								<!-- status since -->
								<td style="font-size:x-small;" data-original-title="<?php echo h($this->Time->format($host['Hoststatus'][0]['Hoststatus']['last_state_change'], $this->Auth->user('dateformat'), false, $this->Auth->user('timezone'))); ?>" data-placement="bottom" rel="tooltip" data-container="body">
						...
						<?php else: ?>
							<tr>
								<td style="font-size:xx-small;" class="text-center" colspan="8"><?php echo __('This Hostgroup has no Hosts'); ?></td>
							</tr>
						<?php endif; ?>
					<?php endforeach; ?>
