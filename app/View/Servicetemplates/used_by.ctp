		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-1" data-widget-editbutton="false" >
				<header>
					...
					<h2><?php echo __('Servicetemplate'); ?> <strong><?php echo h($servicetemplate['Servicetemplate']['name']); ?></strong> <?php echo __('is used by the following');?> <?php echo $this->Utils->pluralize($all_hosts, __('host'), __('hosts'));?> (<?php echo sizeof($all_hosts); ?>):</h2>
									...
									<tr>
										<td class="bg-color-lightGray" colspan="2">
											...
											<?php if($this->Acl->hasPermission('browser', 'hosts')): ?>
												<a class="padding-left-5 txt-color-blueDark" href="/hosts/browser/<?php echo $host['Host']['id']; ?>"><?php echo h($host['Host']['name']); ?> (<?php echo h($host['Host']['address']); ?>)</a>
											<?php else: ?>
											...
											<tr>
												...
												<td class="text-center" style="width: 15px;">
													<?php if($this->Acl->hasPermission('edit', 'services') && $hasEditPermission): ?>
														<input type="checkbox" class="massChange" servicename="<?php echo $service_name; ?>" value="<?php echo $service['Service']['id']; ?>" />
													<?php endif;?>
												</td>
												<td>
