...
<header>
	...
	<div class="widget-toolbar" role="menu">
		<?php if($this->Acl->hasPermission('delete')): ?>
			<?php echo $this->Utils->deleteButton(null, $service['Service']['id']);?>
		<?php endif; ?>
		<?php if($this->Acl->hasPermission('browser')): ?>
			<a href="/services/browser/<?php echo $service['Service']['id']; ?>" class="btn btn-default btn-xs"><i class="fa fa-cog"></i> <?php echo __('Browser');?></a>
		<?php endif; ?>
...
<div id="tab1" class="tab-pane fade active in">
	...
	<div class="form-group">
		<label class="col col-md-2 control-label" for="ServiceName"><?php echo __('Host'); ?></label>
		<div class="col col-xs-10 padding-top-5"><?php echo $service['Host']['name']; ?></div>
		<?php echo $this->Form->input('Service.host_id', ['type' => 'hidden', 'value' => $service['Service']['host_id']]); ?>
	</div>
	<?php
	if($service['Service']['service_type'] == MK_SERVICE):
		echo $this->Form->input('Service.servicetemplate_id', ['type' => 'hidden', 'value' => $service['Service']['servicetemplate_id']]);
		?>
		<div class="form-group">
			<label class="col col-md-2 control-label" for="ServiceName"><?php echo __('Servicetemplate'); ?></label>
		...
		echo $this->Form->input('Service.servicetemplate_id', [
			...
			'data-placeholder' => __('Please select...'),
			'class' => 'chosen',
			'label' => '<a href="/servicetemplates/edit/' . $service['Service']['servicetemplate_id'] . '"><i class="fa fa-cog"></i> </a>' . __('Servicetemplate'),
	...
	if($service['Service']['service_type'] == MK_SERVICE): ?>
	<div class="form-group">
		<label class="col col-md-2 control-label" for="ServiceName"><?php echo __('Name'); ?></label>
	...
	echo $this->Form->input('Service.Servicegroup', [
		...
		'selected' => Hash::extract($service['Servicegroup'], '{n}.id', '{n}.id'),
		'wrapInput' => 'col col-xs-8',
		'style' => 'width: 100%',
		'multiple' => true,
	]);
	...
	<?php if($ContactsInherited['inherit'] === true):
		...
		<span class="text-info"><?php echo __('Untick to disable');?><input type="checkbox" id="inheritContacts" checked="checked" /></span>
		<?php
			$contactOptions = [
