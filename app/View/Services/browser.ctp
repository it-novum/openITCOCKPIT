		...
		<h1 class="page-title <?php echo $this->Status->ServiceStatusColor($service['Service']['uuid']); ?>">
			...
			?><span>
				...
				<?php if($this->Acl->hasPermission('browser', 'hosts')): ?>
					<a href="/hosts/browser/<?php echo $service['Host']['id']; ?>"><?php echo h($service['Host']['name']); ?> (<?php echo h($service['Host']['address']); ?>)</a>
				<?php else: ?>
				...
				<div class="tab-content padding-10">
					<div id="tab1" class="tab-pane fade active in">
						...
						<p><?php echo __('The last system check occurred at'); ?> <strong><?php echo $this->Time->format($this->Status->sget($service['Service']['uuid'], 'last_check'), $this->Auth->user('dateformat'), false, $this->Auth->user('timezone')); ?></strong>
							...
							<?php if($this->Monitoring->checkForDowntime($this->Status->sget($service['Service']['uuid'], 'scheduled_downtime_depth'))):?>
								...
								<span class="fa-stack fa-lg">
									...
									<?php echo __('The service is currently in a planned maintenance period.'); ?><br /><br />
								</p>
							<?php endif;?>
							<?php if($this->Status->get($service['Host']['uuid'], 'current_state') > 0):?>
								<p class="parentstatus padding-left-10">
									<?php echo __('problem with associated host');?> <a href="/hosts/browser/<?php echo $service['Host']['id']; ?>"><?php echo h($service['Host']['name']);?></a> <?php echo __('detected');?>
									<br />
									...
									<code class="no-background <?php echo $this->Status->HostStatusColor($service['Host']['uuid']); ?>">(<?php echo h($this->Status->get($service['Host']['uuid'], 'output'));?>)</code>
								</p>
							<?php endif; ?>
							...
							<table class="table table-bordered">
									...
									<td>
										<?php if($this->Status->sget($service['Service']['uuid'], 'active_checks_enabled') == 1 && $service['Host']['satellite_id'] == 0 && $this->Status->sget($service['Service']['uuid'], 'active_checks_enabled') !== null): ?>
											...
											<?php if($this->Status->sget($service['Service']['uuid'], 'latency') > 1): ?>
												<span class="text-muted" title="<?php echo __('Check latency'); ?>">(+<?php echo $this->Status->sget($service['Service']['uuid'], 'latency'); ?>)</span>
											<?php endif; ?>
										<?php else:?>
											<?php
								...
								<tr>
									...
									<td><code class="no-background <?php echo $this->Status->ServiceStatusColor($service['Service']['uuid']); ?>"><?php echo $this->Status->sget($service['Service']['uuid'], 'output'); ?></code></td>
								</tr>
								<tr>
									<td><strong><?php echo __('Performance data'); ?>:</strong></td>
									<td><code class="no-background <?php echo $this->Status->ServiceStatusColor($service['Service']['uuid']); ?>"><?php echo $this->Status->sget($service['Service']['uuid'], 'perfdata'); ?></code></td>
								</tr>
								</tbody>
							</table>
							<dl>
								...
								<?php if(!empty($long_output)): ?>
									...
									<dd><div id="nag_longout_preview"><?php echo $this->Bbcode->nagiosNl2br(substr(strip_tags($this->Bbcode->asHtml($long_output)), 0, 200)); ?><a href="javascript:void(0);" id="nagShowLongOutput" ><?php echo __('...read more'); ?></a></div>
								...
								<?php else:?>
									<dd> <code class="no-background <?php echo $this->Status->ServiceStatusColor($service['Service']['uuid']); ?>"><?php echo __('No long output available'); ?></code></dd>
								<?php endif;?>
							</dl>
						...
						<div id="tab4" class="tab-pane fade">
							...
							<h5><span class="nag_command" data-toggle="modal" data-target="#nag_command_schedule_downtime"><i class="fa fa-power-off"></i> <?php echo __('Set planned maintenance times'); ?> </span><br /></h5>
							<?php if($this->Status->sget($service['Service']['uuid'], 'current_state') > 0):?>
								<h5><span class="nag_command" data-toggle="modal" data-target="#nag_command_ack_state"><i class="fa fa-user"></i> <?php echo __('Acknowledge service status'); ?> </span><br /></h5>
							<?php endif;?>
							<h5><span class="nag_command" data-toggle="modal" data-target="#nag_command_flap_detection"><i class="fa fa-adjust"></i> <?php echo __('Enables/disables flap detection for a particular service'); ?> </span><br /></h5>
							...
							<h5><span class="nag_command" data-toggle="modal" data-target="#nag_command_custom_notification"><i class="fa fa-envelope"></i> <?php echo __('Send custom service notification'); ?> </span><br /></h5>
						</div>
					</div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="row">
					<div class="txt-color-red padding-bottom-20" id="validationErrorServiceDowntime" style="display:none;"><i class="fa fa-exclamation-circle"></i> <?php echo __('Please enter a valide date');?></div>
					<?php
					echo $this->Form->create('CommitServiceDowntime', array(
						'class' => 'form-horizontal clear',
					)); ?>
					<?php echo $this->Form->input('comment', ['value' => __('In progress'), 'label' => __('Comment').':']); ?>
					<!-- from -->
					<div class="form-group">
						<label class="col col-md-2 control-label" for="CommitServiceDowntimeFromDate"><?php echo __('From');?>:</label>
						<div class="col col-xs-5" style="padding-right: 0px;">
							<input type="text" id="CommitServiceDowntimeFromDate" value="<?php echo date('d.m.Y'); ?>" class="form-control" name="data[CommitServiceDowntime][from_date]">
						</div>
						<div class="col col-xs-5" style="padding-left: 0px;">
							<input type="text" id="CommitServiceDowntimeFromTime" value="<?php echo date('h:m'); ?>" class="form-control" name="data[CommitServiceDowntime][from_time]">
						</div>
					</div>

					<!-- to -->
					<div class="form-group">
						<label class="col col-md-2 control-label" for="CommitServiceDowntimeToDate"><?php echo __('To');?>:</label>
						<div class="col col-xs-5" style="padding-right: 0px;">
							<input type="text" id="CommitServiceDowntimeToDate" value="<?php echo date('d.m.Y', strtotime('+3 days')); ?>" class="form-control" name="data[CommitServiceDowntime][to_date]">
						</div>
						<div class="col col-xs-5" style="padding-left: 0px;">
							<input type="text" id="CommitServiceDowntimeToTime" value="<?php echo date('h:m'); ?>" class="form-control" name="data[CommitServiceDowntime][to_time]">
						</div>
					</div>
			...
			<div class="modal-footer">
				<a href="<?php echo Router::url(['controller' => 'systemdowntimes', 'action' => 'addServicedowntime', 'service_id' => $service['Service']['id']]);?>" class="btn btn-primary pull-left"><i class="fa fa-cogs"></i> <?php echo __('More options'); ?></a>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="row">
					<?php
					echo $this->Form->create('CommitCustomServiceNotification', array(
						'class' => 'form-horizontal clear',
					)); ?>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="row">
					<?php
					echo $this->Form->create('CommitServicestateAck', array(
						'class' => 'form-horizontal clear',
					)); ?>
					<?php echo $this->Form->input('comment', ['value' => __('In progress'), 'label' => __('Comment').':']); ?>
