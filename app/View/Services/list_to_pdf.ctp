						...
						<?php if(!empty($service['ServiceData'])): ?>
							<?php foreach ($service['ServiceData'] as $key => $servicedata): ?>
								...
								<tr> 
									...
									<!-- Status Since -->
									<td class="font-xs" data-original-title="<?php echo h($this->Time->format($servicedata['Servicestatus']['last_state_change'], $this->Auth->user('dateformat'), false, $this->Auth->user('timezone'))); ?>" data-placement="bottom" rel="tooltip" data-container="body">
