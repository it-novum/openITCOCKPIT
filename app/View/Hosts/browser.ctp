                                        ...
                                        <div class="custom-scroll">
                                            <?php if (!empty($disabled_services)): ?>
                                                <table class="table table-bordered" id="host_browser_disabled_service_table">
                                                    ...
                                                    <tbody>
                                                    <?php foreach ($disabled_services as $disabled_service): ?>
                                                        <tr>
                                                            <td class="text-center width-90">
                                                                <?php
                                                                if ($this->Status->sget($disabled_service['Service']['uuid'], 'is_flapping') == 1):
                                                                    echo $this->Monitoring->serviceFlappingIconColored($this->Status->sget($disabled_service['Service']['uuid'], 'is_flapping'), '', $this->Status->sget($disabled_service['Service']['uuid'], 'current_state'));
                                                                else:
                                                                    $currentState = -1;
                                                                    if (isset($servicestatus[$service['Service']['uuid']]['Servicestatus']['current_state'])) {
                                                                        $currentState = $servicestatus[$disabled_service['Service']['uuid']]['Servicestatus']['current_state'];
                                                                    }
                                                                    echo $this->Status->humanServiceStatus($disabled_service['Service']['uuid'], '/services/browser/'.$disabled_service['Service']['id'], null, $currentState, 'color: transparent;')['html_icon'];
                                                                endif;
                                                                ?>
                                                            </td>
                                                            <td class="text-center">
                                                                <?php if ($this->Monitoring->checkForAck($this->Status->sget($disabled_service['Service']['uuid'], 'problem_has_been_acknowledged'))): ?>
                                                                    <?php if ($servicestatus[$disabled_service['Service']['uuid']]['Servicestatus']['acknowledgement_type'] == 1): ?>
                                                                        <i class="fa fa-user"
                                                                           title="<?php echo __('Acknowledgedment'); ?>"></i>
                                                                    <?php else: ?>
                                                                        <i class="fa fa-user-o"
                                                            ...
                                                            <td class="text-center">
                                                                <?php if ($this->Monitoring->checkForDowntime($this->Status->sget($disabled_service['Service']['uuid'], 'scheduled_downtime_depth'))): ?>
                                                                    <i class="fa fa-power-off"></i>
                                                                <?php endif; ?>
                                                            </td>
                                                            <td class="text-center">
                                                                <?php if ($this->Monitoring->checkForServiceGraph($disabled_service['Host']['uuid'], $disabled_service['Service']['uuid'])): ?>
                                                                    <a class="txt-color-blueDark"
                                                                       href="/services/grapherSwitch/<?php echo $service['Service']['id']; ?>"><i
                                                                                class="fa fa-area-chart fa-lg popupGraph"
                                                                                host-uuid="<?php echo $disabled_service['Host']['uuid']; ?>"
                                                                                service-uuid="<?php echo $disabled_service['Service']['uuid']; ?>"></i></a>
                                                                <?php endif; ?>
                                                            </td>
                                                            <td class="text-center">
                                                                <?php if (($this->Status->sget($disabled_service['Service']['uuid'], 'active_checks_enabled') == 0 && $this->Status->sget($disabled_service['Service']['uuid'], 'active_checks_enabled') !== null) || (isset($disabled_service['Host']['satellite_id'])) && $disabled_service['Host']['satellite_id'] > 0): ?>
                                                                    <strong title="<?php echo __('Passively transferred service'); ?>">P</strong>
                                                                <?php endif; ?>
                                                            </td>
                                                            <td>
                                                                <?php
                                                                $serviceName = $disabled_service['Service']['name'];
                                                                if ($serviceName === null || $serviceName === ''):
                                                                    $serviceName = $disabled_service['Servicetemplate']['name'];
                                                                endif;
                                                                ?>
                                                                <?php if ($this->Acl->hasPermission('browser', 'services')): ?>
                                                                    <a href="/services/browser/<?php echo $disabled_service['Service']['id']; ?>"><?php echo h($serviceName); ?></a>
                                                                <?php else: ?>
                                                                    <?php echo h($serviceName); ?>
                                                                <?php endif; ?>
                                                            </td>
                                                            <td data-original-title="<?php echo h($this->Time->format($this->Status->sget($disabled_service['Service']['uuid'], 'last_hard_state_change'), $this->Auth->user('dateformat'), false, $this->Auth->user('timezone'))); ?>"
                                                                data-placement="bottom" rel="tooltip" data-container="body">
                                                                <?php echo h($this->Utils->secondsInHumanShort(time() - strtotime($this->Status->sget($service['Service']['uuid'], 'last_hard_state_change')))); ?>
                                                            </td>
                                                            <td><?php echo h($this->Status->sget($disabled_service['Service']['uuid'], 'output')); ?></td>
                                                        </tr>
                                                    <?php endforeach; ?>
                                                    </tbody>
                                                </table>
