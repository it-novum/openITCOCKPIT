...
//	confirmation.
?>
<?php $this->Paginator->options(['url' => Hash::merge($this->params['named'], $this->params['pass'], ['Listsettings' => $ServicecheckListsettings])]); ?>
<div class="row">
				...
				<header>
					<div class="widget-toolbar" role="menu">
						...
						<ul class="dropdown-menu arrow-box-up-right pull-right">
							...
							<li style="width: 100%;"><a href="javascript:void(0)" class="select_datatable text-left" my-column="5"><input type="checkbox" class="pull-left" /> &nbsp; <?php echo __('Performance data'); ?></a></li>
						</ul>
					...
					<div id="switch-1" class="widget-toolbar" role="menu">
						...
						<div class="widget-toolbar pull-left" role="menu">
							...
							<div class="btn-group">
								...
								<input type="hidden" value="<?php if(isset($ServicecheckListsettings['limit'])): echo $ServicecheckListsettings['limit']; endif; ?>" id="listoptions_hidden_limit" name="data[Listsettings][limit]" />
							</div>
						...
						<table id="servicechecks_list" class="table table-striped table-bordered smart-form" style="">
							<thead>
								<tr>
									...
									<th class="no-sort"><?php echo $this->Utils->getDirection($order, 'Servicecheck.state'); echo $this->Paginator->sort('Servicecheck.state', __('State')); ?></th>
									<th class="no-sort"><?php echo $this->Utils->getDirection($order, 'Servicecheck.start_time'); echo $this->Paginator->sort('Servicecheck.start_time', __('Date')); ?></th>
									<th class="no-sort"><?php echo $this->Utils->getDirection($order, 'Servicecheck.current_check_attempt'); echo $this->Paginator->sort('Servicecheck.current_check_attempt', __('Check attempt')); ?></th>
									<th class="no-sort"><?php echo $this->Utils->getDirection($order, 'Servicecheck.state_type'); echo $this->Paginator->sort('Servicecheck.state_type', __('State type')); ?></th>
									<th class="no-sort"><?php echo $this->Utils->getDirection($order, 'Servicecheck.output'); echo $this->Paginator->sort('Servicecheck.output', __('Service output')); ?></th>
									<th class="no-sort"><?php echo $this->Utils->getDirection($order, 'Servicecheck.perfdata'); echo $this->Paginator->sort('Servicecheck.perfdata', __('Performance data')); ?></th>
								</tr>
							</thead>
							<tbody>
								...
								<?php foreach($all_servicechecks as $servicecheck): ?>
									<tr>
										...
										<td class="text-center"><?php echo h($servicecheck['Servicecheck']['current_check_attempt']);?>/<?php echo h($servicecheck['Servicecheck']['max_check_attempts']);?></td>
